...
onload = async function () {
  ...
  if (cookieExp <= new Date() || (cookieExp - new Date()) / 60000 > 60) {
    fetch(`${backOrigin}/api/Account/UpdateToken`, {
      method: "POST",
      credentials: "include",
    })
      .then((res) => {
    ...
    for (let i of usersOfGroup) {
      if (i.userName != myusername) {
        if (Object.keys(rtcObjectsControl).length == 0)
          ...
          controls = await buildSdpOffer(
            video,
            rtcObjectsControl[Object.keys(rtcObjectsControl)[0]].getUserStream()
          );
        }
        controls.Rtc.ontrack = function (e) {
          if (videonode == null || videonode.srcObject != e.streams[0]) {
            ...
            mystreamobj =
              rtcObjectsControl[
                Object.keys(rtcObjectsControl)[0]
              ].Rtc.getLocalStreams()[0];
        ...
        signal.invoke(
          ...
          myusername,
          getCookie("fullname"),
          JSON.stringify(controls.Rtc.localDescription.sdp),
          i.userName
        );
      }
    }
  ...
  signal.on("getSdpReplyOfUser", async function (username, sdp1) {
    rtcObjectsControl[username].Rtc.setRemoteDescription({
      type: "answer",
      sdp: JSON.parse(sdp1),
    });
  });
  ...
  videobtn.onclick = async function () {
    ...
    if (Object.keys(rtcObjectsControl).length != 0) {
      ...
      if (video.srcObject == null) {
        let userstreamm = await navigator.mediaDevices.getUserMedia({
          video: true,
          audio: true,
        });
  ...
  async function toggleScreen() {
    ...
    if (document.querySelector(".screenbtn").classList.contains("cancelline")) {
      ...
      for (let i in rtcObjectsControl) {
        let videosender = rtcObjectsControl[i].Rtc.getSenders().find(
          (x) => x.track != null && x.track.kind == "video"
        );
